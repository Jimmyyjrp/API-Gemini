...
def handle_message(event):
    ...
    # 👉 รองรับทั้ง "ชื่อเรื่อง:" และแบบพูดแทรก เช่น แนะนำเรื่องนี้เลย "..."
    clean_answer = re.sub(r'[^\S\r\n]*(ชื่อ[ ]?เรื่อง)[ ]*:[^\S\r\n]*', '🎬 ชื่อเรื่อง: ', clean_answer, flags=re.IGNORECASE)
    clean_answer = re.sub(r'แนะนำเรื่องนี้เลย\s*["“](.*?)["”]', r'🎬 ชื่อเรื่อง: \1', clean_answer)

    # หัวข้อทั่วไปของอนิเมะ
    ...
    # หัวข้อเกี่ยวกับเพลงในอนิเมะ
    clean_answer = re.sub(r"[^\S\r\n]*(ชื่อ[ ]?เพลง|เพลง|เพลิง)[ ]*:[^\S\r\n]*", "🎶 ชื่อเพลง: ", clean_answer, flags=re.IGNORECASE)
    ...
    # แปลงลิงก์ markdown ให้อยู่คนละบรรทัด
    clean_answer = re.sub(r"\[(.*?)\]\((.*?)\)", r"\1\n\2", clean_answer)
